...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('demote')
    .setDescription('Voeg een lagere rol toe aan een gebruiker.')
    .addUserOption((option) =>
      option.setName('user').setDescription('De gebruiker').setRequired(true)
    )
    .addRoleOption((option) =>
      option.setName('role').setDescription('De lagere rol').setRequired(true)
    )
  ...
  async execute(interaction) {
    ...
    if (!targetMember.roles.cache.has(role.id)) {
      await targetMember.roles.add(role, `Demotie door ${interaction.user.tag}: ${uitleg}`);
    }
    ...
    const embed = createLogEmbed({
      ...
      fields: [
        ...
        { name: 'Nieuwe lagere rol', value: `${role}`, inline: true },
    ...
    return interaction.reply({
      content: 'Demotie verwerkt en gelogd.',
      ephemeral: true,
    });
  },
};
